...
interface AccordionPanelProps {
  ...
  /** 서버 과부하(서킷 브레이커) 시 게시판에 "집계중" 메시지 표시 (기존 데이터 fallback 유지) */
...
export function AccordionPanel({
  ...
  return (
    <div className="group relative bg-white/60 backdrop-blur-sm border border-white/70 shadow-[0_8px_32px_rgba(0,0,0,0.1)] rounded-sm overflow-hidden transition-shadow duration-300 hover:shadow-[0_8px_32px_rgba(255,255,255,0.1)]">
      ...
      >
        <div className="pb-4">
          <BoardTable type={title} applications={applications} boardOverloaded={boardOverloaded} isGracePeriod={isGracePeriod} highlightCount={
            capacity == null
              ? undefined
              : typeof capacity === 'object'
                ? capacity.limit + capacity.special_count
